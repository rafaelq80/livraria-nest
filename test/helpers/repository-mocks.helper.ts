...
import { MockRepository, CategoriaMock, ProdutoMock, EditoraMock, AutorMock } from "./types.helper";
...
export class RepositoryMocks {
  ...
  static createRepositoryMock<T = unknown>(mockData: T | T[]): MockRepository<T> {
    ...
    return {
      ...
      save: jest.fn().mockImplementation((entity: Partial<T>) => 
  ...
  static createProdutoRepositoryMock(
    produtos: ProdutoMock[] = [EntityMocks.createProdutoMock()]
  ): MockRepository<ProdutoMock> {
    return this.createRepositoryMock(produtos);
  }
}